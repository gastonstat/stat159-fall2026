...
is related to the lecture [Case Study: Global Temperature](../../lectures/09-global-temp/09-global-temp-slides.html) from
Sep-16.
...
- One report document, saved in a `report/` file, that provides an executive summary
...
the project, and not include any other file or folder that those needed for the project.

Likewise, we will assess how you put in practice all the concepts covered so far:
naming files, file paths, project organization, comments, documentation, etc.

To ensure that extraneous files are ignored by git, add a `.gitignore` where you 
